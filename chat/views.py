...
class ChatRoomViewSet(viewsets.ModelViewSet):
    ...
    def get_or_create(self, request):
        """
        Get or create a chat room for a specific offer and craftsman
        ...
        # New domain rule: customer must have an inquiry for this offer
        if not Inquiry.objects.filter(offer=job, customer=request.user).exists():
            return Response(
                {'error': 'Nur Kunden mit einer Anfrage zu diesem Angebot dürfen chatten.'},
                status=status.HTTP_403_FORBIDDEN
            )
        # And craftsman must be the owner of the offer
        if job.craftsman_id != int(craftsman_id):
            return Response(
                {'error': 'Ungültiger Handwerker für dieses Angebot.'},
                status=status.HTTP_400_BAD_REQUEST
            )
